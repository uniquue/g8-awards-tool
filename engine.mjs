...
export function calculate(employees,settings={}) {
 ...
 const unmapped=employees.filter(e=>e.dir==='Unmapped');if(unmapped.length)errors.push(`${unmapped.length} employees have unrecognized Org Component codes: ${[...new Set(unmapped.map(e=>e.org||'(blank)'))].join(', ')}. Review their directorate mapping.`);
