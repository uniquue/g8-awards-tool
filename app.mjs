...
function draw(){const agencySalary=employees.reduce((n,e)=>n+e.salary,0),selected=employees.filter(e=>selectedDirs.has(e.dir)),selectedSalary=selected.reduce((n,e)=>n+e.salary,0),pooled=settings.allocationMode==='pooled';const all=calculate(pooled?selected:employees,{...settings,budget:pooled&&settings.budget!=null?Math.floor(settings.budget*(agencySalary?selectedSalary/agencySalary:0)):settings.budget}),rows=all.rows.filter(e=>selectedDirs.has(e.dir)),salary=rows.reduce((n,e)=>n+e.salary,0),budget=[...selectedDirs].reduce((n,d)=>n+(all.directorateBudgets[d]||0),0),cash=rows.reduce((n,e)=>n+e.cash,0);result={...all,...budgetBreakdown(salary,settings),rows,totalSalary:salary,budget,cash,unallocated:budget-cash};$('message').textContent=all.errors.join('\n');drawViews();}
function drawViews(){
 ...
 $('export').textContent=settings.allocationMode==='pooled'?'Export selected pool workbook':'Export full workbook';
 $('scope').textContent=`Showing ${rows.length} of ${employees.length} employees · ${scopeLabel()} · ${settings.allocationMode==='pooled'?'Pooled budget':'Separate budgets'}`;

 $('metrics').innerHTML=[['Employees',rows.length,`${rows.filter(e=>e.score===0).length} unrated`],['Cash allocated',money(cash),`${money(pool)} selected cash budget`],['Cash remaining',money(pool-cash),`${money(nrbBudget-sum('nrb'))} NRB remaining`],['QSI selections',`${rows.filter(e=>e.type==='QSI').length} / ${result.qsiSlots}`,'Selected employees / organization-wide slots']].map(([a,b,c],i)=>`<article class="metric" id="metric-${i}"><div class="card-heading"><span>${a}</span><button data-print="metric-${i}" aria-label="Print ${a}">Print</button></div><b>${b}</b><small>${c}</small></article>`).join('');
...
add('Dashboard',['Metric','Value'],[['Fiscal year',fiscalYear()],['Source',filename],['Employees',exportEmployees.length],['Allocation mode',settings.allocationMode],['Directorates',[...new Set(exportEmployees.map(e=>e.dir))].join(', ')],['Total award budget',result.totalBudget],['Supervisory reserve',result.supervisoryBudget],['Rating budget',result.ratingBudget],['Selected cash pool',result.budget],['Cash allocated',result.cash],['Unallocated',result.unallocated],['NRB budget',result.nrbBudget],['NRB allocated',result.nrb],['QSI selected',result.qsi],['QSI slots',result.qsiSlots],...result.errors.map(e=>['Review flag',e])]);add('Work Tab',headers,result.rows.map(record));add('Budget Summary',summaryHeaders,summaryRows(result.rows,result.directorateBudgets));add('GS Grade Summary',gradeHeaders,gradeSummary(result));for(const [name,filter]of Object.entries(filters))add(name,headers,result.rows.filter(filter).map(record));add('Inputs',['Source row','Directorate','Name','Original score','Adjusted score','Award','TOA fraction','Months','NRB'],exportEmployees.map(e=>[e.sourceRow,e.dir,e.name,e.rawScore,e.score,e.type,e.timeShare,e.months,e.nrb]));add('Calculation Notes',['Setting','Value'],Object.entries(settings).map(([k,v])=>[k,v??'Automatic']).concat([['Proration','Months / 12'],['Rounding','Whole-dollar largest remainder; stable uploaded row order breaks ties'],['Time off','40 * score / 5 * time fraction * proration; two decimals'],['Reports','TOA and cash reports overlap with combined awards'],['Review','Draft calculations; selections require human review']]));const blob=new Blob([await wb.xlsx.writeBuffer()],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='G8-Awards-Review-'+fiscalYear()+'.xlsx';a.click();setTimeout(()=>URL.revokeObjectURL(url),10000);}catch(e){$('message').textContent='Export failed: '+e.message;}finally{button.disabled=false;}});
